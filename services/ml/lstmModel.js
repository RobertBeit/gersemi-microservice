...
export const buildAndTrainLSTMModel = async (features, epochs = 50, batchSize = 32, customCallbacks = {}) => {
  ...
  // Convert to float32 tensor with one-hot encoding
  const yTensor = tf.tensor2d(oneHotLabels, [oneHotLabels.length, 3], 'float32');
  ...
  const yTrain = yTensor.slice([0, 0], [splitIdx, 3]);
  
  const xVal = xTensor.slice([splitIdx, 0, 0], [sequences.length - splitIdx, sequenceLength, inputDim]);
  const yVal = yTensor.slice([splitIdx, 0], [sequences.length - splitIdx, 3]);
  ...
  const defaultCallbacks = {
    onEpochEnd: (epoch, logs) => {
      ...
      trainHistory.acc.push(logs.acc);
      trainHistory.val_loss.push(logs.val_loss);
      trainHistory.val_acc.push(logs.val_acc);
    }
  };
